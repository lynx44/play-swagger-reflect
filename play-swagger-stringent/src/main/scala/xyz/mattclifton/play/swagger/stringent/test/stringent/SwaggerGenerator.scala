...
//          println(s"response types: $responseTypes")
          val newDoc = Some(finalDoc).map(doc => {
            ...
            val updatedResponses = responseTypes.foldLeft(responses)((response, arg) => arg.typeSymbol.name.decodedName.toString match {
              ...
              case "InternalServerErrorResult" => mergeByResponseCode(play.api.http.Status.INTERNAL_SERVER_ERROR, response)
              case "NotImplementedResult" => mergeByResponseCode(play.api.http.Status.NOT_IMPLEMENTED, response)
              case "BadGatewayResult" => mergeByResponseCode(play.api.http.Status.BAD_GATEWAY, response)
              case "ServiceUnavailableResult" => mergeByResponseCode(play.api.http.Status.SERVICE_UNAVAILABLE, response)
              case "GatewayTimeoutResult" => mergeByResponseCode(play.api.http.Status.GATEWAY_TIMEOUT, response)
              case "HttpVersionNotSupportedResult" => mergeByResponseCode(play.api.http.Status.HTTP_VERSION_NOT_SUPPORTED, response)
              case "InsufficientStorageResult" => mergeByResponseCode(play.api.http.Status.INSUFFICIENT_STORAGE, response)
              ...
              case "InternalServerErrorWithContent" => mergeByResponseCode(play.api.http.Status.INTERNAL_SERVER_ERROR, response, Some(arg))
              case "NotImplementedWithContent" => mergeByResponseCode(play.api.http.Status.NOT_IMPLEMENTED, response, Some(arg))
              case "BadGatewayWithContent" => mergeByResponseCode(play.api.http.Status.BAD_GATEWAY, response, Some(arg))
              case "ServiceUnavailableWithContent" => mergeByResponseCode(play.api.http.Status.SERVICE_UNAVAILABLE, response, Some(arg))
              case "GatewayTimeoutWithContent" => mergeByResponseCode(play.api.http.Status.GATEWAY_TIMEOUT, response, Some(arg))
              case "HttpVersionNotSupportedWithContent" => mergeByResponseCode(play.api.http.Status.HTTP_VERSION_NOT_SUPPORTED, response, Some(arg))
              case "InsufficientStorageWithContent" => mergeByResponseCode(play.api.http.Status.INSUFFICIENT_STORAGE, response, Some(arg))
            })
